...
# Initial value
x0 = np.loadtxt(f'{os.path.dirname(__file__)}/data/xbase_2D.dat')
M0 = np.loadtxt(f'{os.path.dirname(__file__)}/data/Mbase_2D.dat')
rho0 = np.loadtxt(f'{os.path.dirname(__file__)}/data/rhobase_2D.dat')
v0 = np.zeros((len(M0), 2))
for idim in range(2):
    v0[:,idim] = np.loadtxt(f'{os.path.dirname(__file__)}/data/vbase{idim}_2D.dat')

# Interpolation reference
xinterp = np.loadtxt(f'{os.path.dirname(__file__)}/data/xinterp_2D.dat')
Minterp_ref = np.loadtxt(f'{os.path.dirname(__file__)}/data/Minterp_2D.dat')
rhointerp_ref = np.loadtxt(f'{os.path.dirname(__file__)}/data/rhointerp_2D.dat')
vinterp_ref = np.zeros((len(Minterp_ref), 2))
for idim in range(2):
    vinterp_ref[:,idim] = np.loadtxt(f'{os.path.dirname(__file__)}/data/vinterp{idim}_2D.dat')
...
vinterp = np.zeros((len(Minterp), 2))
for idim in range(2):
...
vinterp_sp = np.zeros((len(Minterp_sp), 2))
for idim in range(2):
